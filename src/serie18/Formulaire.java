...
public class Formulaire extends JPanel{
	...
	private JTextField commentaires, dureeInstallation, reference;
	...
	public Formulaire(FenetrePrincipale f){ //la classe formulaire recoit en argument "connection" pour se connecter a la BD une fois que le formulaire se lance
		...
		p = new PanneauDate(); 
		add(p);
		p.setEnabled(false); // désactiver par défaut
		initialiserTableaux(); //les jcombobox ont besoin  de tableau pour afficher les contenu de la DB
			...
			if (e.getSource().equals(insertion)){
				...
				if(valide()) {
					try {
						...
						int newID = (int) AccessBDGen.creerListe1Colonne(preparation)[0]; // on prend le premier element du tableau qui est renvoyé (façon racourci de prendre la premiere cellule du tableau et de la metre dans NEWID
						newID++; // incrémenter l ID sinon erreur (2x un enregistrement avec la même clé ça pète mdr !!!
						...
						String com;
						com = commentaires.getText(); // on va rechercher ce que l user a inserer dans commentaires
						if(com.equals("")){
							preparation.setString(4, null); // soit on recupere rien
						}else{
							preparation.setString(4, com); // soit on récupere le commentaire com
						}
